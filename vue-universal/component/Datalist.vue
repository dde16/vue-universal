<template>
    <div class="position-relative display-block" v-clickoff="closeDropdown">
        <v-input
            :class="['w-100']"
            :style="{
                fontSize: $props.size
        ...
        >
            <template v-if="state === State.IDLE">
                <template v-for="({value: item, pos, only}, index) in filtered.annotate()" :key="index">
                    <button
                        :class="pos !== 1 && !only && 'border-b1 border-grey-400'"
                        class="w-100 bg-hover-grey-700"
                        @click="updateItem(item)"
                    >
                        <div class="input h-100 w-100 border-0 bg-transparent" :style="{fontSize: $props.size}">
                            {{item.text}}
                        </div>
                    </button>
                </template>
