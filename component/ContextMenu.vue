<template>
    ...
    >
            <button
                class="display-flex w-100 pd-l3 pd-r4 pd-y2 fg-hover-white items-align-center flex-gap-2"
                v-for="(item, index) in items" :key="index" @click.stop="onClick($event, item)">
...
export default defineComponent({
    ...
    data() {
        ...
        switch(Object.isInstanceOf(data.positionSource, [MouseEvent, HTMLElement, Array])) {
            case Array:
                if(data.positionSource.all(Number.isInteger)){
                ...
                break;
            case HTMLElement:
                const boundingRect = data.positionSource.getBoundingClientRect();

                data.initialPosition = [
                    parseInt((boundingRect.left )),
                    parseInt((boundingRect.top))
                ];
