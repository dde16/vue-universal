        ...
        <slot name="items">
            <li v-for="(item, index) in $data.mappedItems.filter(item => item)" :key="index">
                <slot name="item"  v-bind="item">
                    <button @click.stop="$emit('click:tab', item)" class="tablist__item " :class="item.icon && 'tablist__item--has-icon'" v-context-menu.large.right.rightClick="$props.contextMenu && $props.contextMenu(item)">
